...
// ===== Config =====
const string ChannelName = "instagram";
...
// Porta HTTP (opcional): ASPNETCORE_URLS=http://0.0.0.0:7020
builder.Services.AddEndpointsApiExplorer();
builder.Services.AddSwaggerGen();

builder.Services.AddSingleton<IProducer<string, string>>(_ =>
{
    var cfg = new ProducerConfig { BootstrapServers = kafkaBootstrap, EnableIdempotence = true };
...
app.MapPost("/mock/incoming", async (IncomingDto dto, IProducer<string, string> prod) =>
{
    ...
    await prod.ProduceAsync(topicIn, new Message<string, string> { Key = dto.ConversationId ?? dto.To ?? Guid.NewGuid().ToString("N"), Value = json });
    return Results.Json(
...
sealed class OutboundConsumer : BackgroundService
{
    ...
    public OutboundConsumer(IConsumer<string, string> consumer, IProducer<string, string> producer,
        string topicOut, string topicStatus, string callbackUrl, string? jwt, string channel)
    {
        _consumer = consumer;
        _producer = producer;
        _topicOut = topicOut;
    ...
    {
        var evt = new { message_id = messageId, channel = _channel, status, timestamp = DateTime.UtcNow };
        var json = JsonSerializer.Serialize(evt);

        await _producer.ProduceAsync(_topicStatus, new Message<string, string> { Key = messageId, Value = json }, ct);
